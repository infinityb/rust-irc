...
pub enum IrcConnectionCommand2 {
    RawWrite(Vec<u8>),
    AddWatcher(Box<EventWatcher+Send>),
    AddBundler(Box<Bundler+Send>),
}

impl IrcConnectionCommand2 {
    pub fn raw_write(message: Vec<u8>) -> IrcConnectionCommand2 {
        IrcConnectionCommand2::RawWrite(message)
    }
}

impl fmt::Show for IrcConnectionCommand2 {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            IrcConnectionCommand2::RawWrite(ref s) => write!(f, "RawWrite({})", s),
            IrcConnectionCommand2::AddWatcher(ref ew) => write!(f, "AddWatcher({}(...))", ew.get_name()),
            IrcConnectionCommand2::AddBundler(ref bun) => write!(f, "AddBundler({}(...))", bun.get_name()),
        }
    }
}
...
pub struct IrcConnectionBuf {
    ...
    command_queue: RingBuf<IrcConnectionCommand2>,
...
impl IrcConnectionBuf {
    ...
    pub fn dispatch(&mut self) -> Vec<IrcEvent> {
        while let Some(command) = self.command_queue.pop_front() {
            use self::IrcConnectionCommand2::{RawWrite, AddWatcher, AddBundler};
            match command {
    ...
    pub fn register(&mut self, nick: &str) -> Future<RegisterResult> {
        use self::IrcConnectionCommand2::AddWatcher;
